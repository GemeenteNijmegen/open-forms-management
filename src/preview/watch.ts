...
async function main() {
  ...
  // chokidar 4+ dropped glob support for watch paths, so a real directory is watched and
  // non-.mustache/.ts files are filtered out via `ignored` instead.
  const watcher = chokidar.watch('src', {
    ignored: (filePath: string, stats?: { isFile(): boolean }) =>
      !!stats?.isFile() && !filePath.endsWith('.mustache') && !filePath.endsWith('.ts'),
    persistent: true,
    ignoreInitial: true,
    // Polling instead of native fs events: bind-mounted devcontainer volumes don't reliably
    // propagate inotify events, so a native watcher can silently miss every change.
    usePolling: true,
    interval: 300,
  });
