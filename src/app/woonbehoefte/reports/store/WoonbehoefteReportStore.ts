...
import {
  isReportAvailable, WOONBEHOEFTE_REPORT_ACTIVE_STATUSES, WoonbehoefteReport, WoonbehoefteReportOptions, WoonbehoefteReportStatus,
} from '../domain/WoonbehoefteReport';
import { canonicalReportFilter, reportFiltersEqual, WoonbehoefteReportFilter } from '../filters/WoonbehoefteReportFilter';

const RETENTION_SECONDS = 30 * 24 * 60 * 60;

// QUEUED has no heartbeat, so this only needs to cover how long an async worker invocation can realistically
// take to start. BUILDING's threshold must stay above the worker Lambda's 15-minute hard timeout, otherwise
// a still-running worker between touchBuilding() calls could be failed here.
...
export class WoonbehoefteReportStore {
  ...
  async createQueued(input: CreateQueuedReportInput): Promise<WoonbehoefteReport> {
    const now = new Date();
    const report: WoonbehoefteReport = {
      reportId: randomUUID(),
      filter: canonicalReportFilter(input.filter),
    ...
    logger.debug('Woonbehoefte report queued', { reportId: report.reportId });
    return report;
  }

  async get(reportId: string): Promise<WoonbehoefteReport | undefined> {
    const result = await this.documentClient.send(new GetCommand({ TableName: this.tableName, Key: { reportId } }));
    return result.Item as WoonbehoefteReport | undefined;
  }

  // Report volume and retention are small; one Scan plus an in-memory sort is simpler than a GSI for this.
  async listRecent(limit = 50): Promise<WoonbehoefteReport[]> {
  ...
  async findMatchingActive(filter: WoonbehoefteReportFilter, options: WoonbehoefteReportOptions): Promise<WoonbehoefteReport | undefined> {
    const reports = await this.scanAll();
    return reports.find((report) => isReportAvailable(report)
      && WOONBEHOEFTE_REPORT_ACTIVE_STATUSES.includes(report.status)
      && reportFiltersEqual(report.filter, filter)
      && optionsEqual(report.options, options));
  }
  ...
  async touchBuilding(reportId: string, caseCount?: number): Promise<void> {
    await this.transition(reportId, 'BUILDING', caseCount !== undefined ? { caseCount } : {});
  }

  async markReady(reportId: string, storageKey: string, caseCount: number, warningCount: number): Promise<boolean> {
    return this.transition(
      reportId, 'BUILDING', { status: 'READY', storageKey, caseCount, warningCount, completedAt: new Date().toISOString() },
    );
  }
  ...
  private async transition(
    reportId: string, requiredStatus: WoonbehoefteReportStatus, changes: Partial<WoonbehoefteReport>, guard: { updatedAt?: string } = {},
  ): Promise<boolean> {
    ...
    try {
      ...
      if (isConditionalCheckFailed(error)) {
        logger.debug('Woonbehoefte report transition skipped, status no longer matched', { reportId, requiredStatus });
        return false;
      }
      throw error;
    }
  ...
  private async failStaleReports(reports: WoonbehoefteReport[]): Promise<WoonbehoefteReport[]> {
    const now = Date.now();
  ...
  private async scanAll(): Promise<WoonbehoefteReport[]> {
    const items: WoonbehoefteReport[] = [];
    ...
    do {
      ...
      items.push(...(result.Items ?? []) as WoonbehoefteReport[]);
